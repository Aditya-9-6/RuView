...
Host side: add `SourceKind::RealtekCsi = 7` to
`authenticated_ingest::SourceKind` (additive; does not renumber or reuse
`Realtek = 2`, which stays RTL8720F radar). Add `realtek_csi.rs` to both
...
- Reuses the exact ingestion/authentication/trust-scoring machinery already
  proven for ESP32/MediaTek/Qualcomm/RTL8720F — no new server-side pattern.
...
against the deployment AP (`ruv.net`). A single `RAC1` datagram was first
